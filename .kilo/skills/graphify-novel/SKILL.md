...
**Interaction with Protoversiones:** For this project ('El Imperio de Dracma'), the `chapters/` and `draft/` directories map to `cuentos/` (canonical stories). The `protoversiones/` directory is a non-canonical quarry zone and MUST NOT be included in graph extraction. When building the bible, use only content from `cuentos/` and `docs/`. See `AGENTS.md` for full project rules.
